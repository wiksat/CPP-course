...
them, only to see them broken again the next_ time the implementation is changed.
...
you may want to make sure the `Set()` and `Clear()` calls are right next_ to the
...
value), so the next_ time around, there’s no value to move from -- you’ll get a
...
taken to the offending line. Or, you can just type `C-x`` to jump to the next_
error.
...
(global-set-key [M-down] 'next_-error)
(global-set-key [M-up]  '(lambda () (interactive) (next_-error -1)))
```
