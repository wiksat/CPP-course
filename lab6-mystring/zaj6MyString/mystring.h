//
// Created by wikto on 12.06.2023.
//
...
public:
    ...
    MyString() : string_text(std::string()), string_size(string_text.size()),
                 string_capacity(initialBufferSize_ + string_text.capacity()) { string_text.reserve(initialBufferSize_); }

    MyString(const char text[]) : string_text(text), string_size(string_text.size()),
                                  string_capacity(initialBufferSize_ + string_text.capacity()) {
        string_text.reserve(initialBufferSize_);
    }
