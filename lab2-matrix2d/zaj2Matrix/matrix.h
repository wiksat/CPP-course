...
public:
    TwoDimensionMatrix();
    TwoDimensionMatrix(const TwoDimensionMatrix &matrix2);
    explicit TwoDimensionMatrix(const MatrixElement matrix2[size_][size_]);
public: // methods:
    [[nodiscard]] MatrixElement get(size_t a, size_t b) const;
    static constexpr size_t size(){ return size_;};
    TwoDimensionMatrix &operator=(TwoDimensionMatrix matrix2);
    friend std::ostream &operator<<(std::ostream &output, const TwoDimensionMatrix &matrix2);
    friend std::istream &operator>>(std::istream &input, TwoDimensionMatrix &matrix2);
    TwoDimensionMatrix& operator*=(MatrixElement number); // metoda klasy
    TwoDimensionMatrix operator&&(const TwoDimensionMatrix& matrix) const;
    MatrixElement *operator[](size_t i);
    const MatrixElement *operator[](size_t i) const;

    explicit operator size_t() const { return size_; };
private:// fields:
